(ns flutter-kit.keystore
  (:require [flutter-kit.path :refer [write-file]]
            [me.raynes.fs :refer [split-ext absolute]]))
...
(defn- add-signing-configs [gradle-filepath]
  (let [rgx-buildTypes #"buildTypes\s*\{\s*release\s*\{"
    ...
    (if (nil? signingConfig-finder)
      ...
      ;; add signingConfig
      (write-file gradle-filepath (clojure.string/replace-first gradle rgx-buildTypes str-buildTypes))
  ...
  (let [gradle-filepath "android/app/build.gradle"
    ...
    (if (nil? keyprops-finder)
      ...
      (do
        ...
        (write-file gradle-filepath (clojure.string/replace-first old-gradle #"android ?\{" keyprops))))
...
(defn- keystore? [path]
  (let [keystore-path-expanded (.getPath (absolute path))
